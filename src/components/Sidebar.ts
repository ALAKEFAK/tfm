...
export const Sidebar = Vue.component('sidebar', {
  ...
  data: function() {
    return {
      ...
      'spectator_tab': 'board',
      'spectator_pause': false as boolean | unknown[],
  ...
  methods: {
    ...
    toggleSpectatorPlayPause() {
      PreferencesManager.save('spectator_pause', !this.isSpectatorPaused(), true);
      window.location.reload();
    },
    isSpectatorPaused(): boolean {
      return PreferencesManager.loadBoolean('spectator_pause');
    },
  },
  template: `
<div :class="'sidebar_cont sidebar '+getSideBarClass()">
  ...
  <a v-if="coloniesCount > 0 && !spectatorMode" href="#colonies">
      <div class="sidebar_item sidebar_item_shortcut">
          <i class="sidebar_icon sidebar_icon--colonies"></i>
      </div>
  </a>
  <div v-if="spectatorMode" class="sidebar_item" v-on:click="toggleSpectatorPlayPause()">
    <div class="sidebar_icon play_button">
      <i v-if="isSpectatorPaused()" class="material-icons">play_arrow</i>
