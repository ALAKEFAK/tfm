...
export class TerraCimmeriaBoard extends Board {
  public static newInstance(shuffleTileMode: ShuffleTileOptionType, rng: Random, includeVenus: boolean): TerraCimmeriaBoard {
    ...
    // y=1
    builder.ocean(TITANIUM, TITANIUM).land().land(POWER, POWER).land(PLANT).land(PLANT, STEEL).ocean(PLANT);
    // y=2
    builder.land().land(POWER, POWER).land(POWER, POWER, POWER).land().land(PLANT).land(PLANT).land(PLANT);
    // y=3
    builder.land(STEEL, STEEL).land().land().land(POWER, POWER).land().land().land(DRAW_CARD).land();
    // y=4
    builder.land().land().land().land(STEEL).land(STEEL)
      .land(DRAW_CARD).land().land(STEEL).ocean(DRAW_CARD);
    // y=5
    builder.land(DRAW_CARD, DRAW_CARD).land().land(TITANIUM, STEEL, STEEL).land().land().land(STEEL, STEEL).land().land(STEEL, STEEL);
    // y=6
    builder.land().land(TITANIUM).land(PLANT).land(PLANT, STEEL, STEEL).land(PLANT, PLANT).land(PLANT).ocean(PLANT, PLANT);
    // y=7
    builder.ocean(STEEL, STEEL).land(PLANT).land(TITANIUM).land(DRAW_CARD).land(PLANT, PLANT).ocean(PLANT);
    // y=8
    builder.ocean(PLANT, PLANT).ocean(PLANT, PLANT).ocean(PLANT, PLANT).land(PLANT).ocean(PLANT, PLANT);

    builder.setMustBeLandSpaces(SpaceName.ALBOR_THOLUS_TERRACIMMERIA, SpaceName.APOLLINARIS_MONS, SpaceName.HADRIACUS_MONS, SpaceName.TYRRHENUS_MONS);
