...
import {calculateTRGain} from '../turmoil/TerraformingRatingAccounting';
...
export const SelectHowToPayForProjectCard = Vue.component('select-how-to-pay-for-project-card', {
  ...
  methods: {
    ...
    hasRedsWarning: function() {
      if (this.playerinput.redsWarning !== true) {
        return false;
      }
      return this.card !== undefined && this.trGain() > 0;
    },
    trGain: function(): number {
      return calculateTRGain(this.player, this.card.name);
    },
    redsPenalty: function(): number {
      return this.trGain() * REDS_RULING_POLICY_COST;
    },
  ...
  <section v-trim-whitespace>
    ...
    <div v-if="hasRedsWarning()" class="card-warning">
      <div>Reds party is currently ruling.</div>
      <div>You will lose {{ redsPenalty() }} mc if you play this card.</div>
    </div>
    ...
    <div v-if="showsave === true">
      <Button type="submit" :onClick="saveData" :title="playerinput.buttonLabel" :danger="hasRedsWarning()" />
    </div>
  </section>
</div>`,
});
