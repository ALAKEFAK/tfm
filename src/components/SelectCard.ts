...
export const SelectCard = Vue.component('select-card', {
  ...
  methods: {
    ...
    hasRedsWarning: function(): boolean {
      ...
      return (typeof this.cards === 'object') && (this.cards.cardType === CardType.STANDARD_PROJECT) && (calculateTRGain(this.player, this.cards.name) > 0);
    },
    redsPenalty: function(): number {
      ...
      if (typeof this.cards === 'object') {
        return calculateTRGain(this.player, this.cards.name) * REDS_RULING_POLICY_COST;
      } else {
        return 0;
      }
    },
