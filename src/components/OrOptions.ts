    ...
    this.playerinput.options.forEach((option: any, idx: number) => {
      ...
      this.$data.childComponents.push(
        new PlayerInputFactory().getPlayerInput(
          ...
          // Show save if button label is sell or play card
          option.buttonLabel === 'Sell' || option.buttonLabel === 'Play card',
          false,
        ),
      );
      subchildren.push(
      ...
      if (showOption) {
        ...
        if (this.showsave && this.$data.selectedOption === idx && option.buttonLabel !== 'Sell' && option.buttonLabel !== 'Play card') {
          children.push(
            createElement(
              'div',
              {
