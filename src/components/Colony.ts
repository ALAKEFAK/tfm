...
export const Colony = Vue.component('colony', {
  ...
  methods: {
    ...
    getCubeYPosition: (
      ...
      case ColonyName.EUROPA_REBALANCED:
      case ColonyName.MIRANDA:
        return 166 + offset;
      case ColonyName.PLUTO:
      case ColonyName.PLUTO_REBALANCED:
        return 165 + offset;
      case ColonyName.LUNA:
      case ColonyName.LUNA_REBALANCED:
        return 163 + offset;
      default:
        return 164 + offset;
      }
    ...
    getGanymedeRebalanced: (): string => {
      return ColonyName.GANYMEDE_REBALANCED;
    },
    getEuropa: (): string => {
      return ColonyName.EUROPA;
    },
    getEuropaRebalanced: (): string => {
      return ColonyName.EUROPA_REBALANCED;
    },
    getCeres: (): string => {
      return ColonyName.CERES;
    },
    getCeresRebalanced: (): string => {
      return ColonyName.CERES_REBALANCED;
    },
    getPluto: (): string => {
      return ColonyName.PLUTO;
    },
    getPlutoRebalanced: (): string => {
      return ColonyName.PLUTO_REBALANCED;
    },
    getEnceladus: (): string => {
    ...
    getIoRebalanced: (): string => {
      return ColonyName.IO_REBALANCED;
    },
    getTriton: (): string => {
    ...
    getLunaRebalanced: (): string => {
      return ColonyName.LUNA_REBALANCED;
    },
    getMiranda: (): string => {
    ...
    getColonyContentOffset: (colony: ColonyModel): number => {
      if (colony.name === ColonyName.PLUTO || colony.name === ColonyName.PLUTO_REBALANCED || colony.name === ColonyName.MIRANDA) return -12;
      return 0;
    },
    getIapetus: (): string => {
  ...
  template: `
    <div class="filterDiv colony-card colonies" :class="colony.name.replace('(⚖)', '') + '-background'" v-i18n>
    ...
    <div class="colony-content" :style="'margin-top: ' + getColonyContentOffset(colony) + 'px;'">
      <div v-if="colony.name === getGanymede() || colony.name === getGanymedeRebalanced()" class="resource plant"></div>
      <div v-if="colony.name === getEuropa()" class="resource money">1</div>
      <div v-if="colony.name === getEuropaRebalanced()" class="resource money">2</div>
      ...
      <div v-if="colony.name === getCeres() || colony.name === getCeresRebalanced()" class="resource steel"></div>
      <div v-if="colony.name === getCeres() || colony.name === getCeresRebalanced()" class="resource steel"></div>
      <div v-if="colony.name === getIo() || colony.name === getIoRebalanced()" class="resource heat"></div>
      <div v-if="colony.name === getIo() || colony.name === getIoRebalanced()" class="resource heat"></div>     
      <div v-if="colony.name === getIoRebalanced()" class="resource heat"></div> 
      <div v-if="colony.name === getLuna() || colony.name === getLunaRebalanced()" class="resource money">2</div>
      ...
      <span v-if="colony.name === getPluto() || colony.name === getPlutoRebalanced()" class="white-char" style="margin-left:5px;">+</span>
      <div v-if="colony.name === getPluto() || colony.name === getPlutoRebalanced()" class="resource card card-with-border" style="transform: scale(0.8);margin-left:-2px;"></div>
      <span v-if="colony.name === getPluto() || colony.name === getPlutoRebalanced()" class="white-char">-</span>
      <div v-if="colony.name === getPluto() || colony.name === getPlutoRebalanced()" class="resource card card-with-border" style="transform: scale(0.8);margin-left:-2px;"></div>
      ...
      <br>
      <div v-if="colony.name === getGanymede() || colony.name === getGanymedeRebalanced()" class="resource plant" style="margin-left:20px;"></div>
      ...
      <div v-if="colony.name === getCeres() || colony.name === getCeresRebalanced()" class="resource steel" style="margin-left:20px;"></div>
      <div v-if="colony.name === getLuna() || colony.name === getLunaRebalanced()" class="resource money" style="margin-left:20px;">&nbsp;</div>
      <div v-if="colony.name === getIapetus()" class="tile rating" style="margin-left:20px; transform: scale(0.8); margin-top:-10px;"></div>
      <div v-if="colony.name === getIo() || colony.name === getIoRebalanced()" class="resource heat" style="margin-left:20px;"></div>
      <div v-if="colony.name === getMiranda()" class="resource animal" style="margin-left:20px;"></div>
      <div v-if="colony.name === getPluto() || colony.name === getPlutoRebalanced()" class="resource card card-with-border" style="margin-left:20px;transform: scale(0.8);margin-top: -8px;"></div>
      <div v-if="colony.name === getEuropa() || colony.name === getEuropaRebalanced()" style="height: 20px; visibility: hidden;display: block;" />
      <div v-if="colony.name !== getEuropa() && colony.name !== getEuropaRebalanced() && colony.name !== getMercury() && colony.name !== getIapetus() && colony.name !== getHygiea() && colony.name !== getTitania() && colony.name !== getVenus() && colony.name !== getLeavitt() && colony.name !== getPallas()" class="white-x"></div>
      ...
      <span v-if="colony.name !== getEuropa() && colony.name !== getEuropaRebalanced() && colony.name !== getPluto() && colony.name !== getPlutoRebalanced() && colony.name !== getMercury() && colony.name !== getIapetus() && colony.name !== getHygiea() && colony.name !== getTitania() && colony.name !== getLeavitt() && colony.name !== getPallas()" class="colony-background-color">
        Trade Income
      </span>
      <span v-if="colony.name === getPluto() || colony.name === getPlutoRebalanced()" class="colony-background-color" style="position:relative; top:-3px">
        Trade Income
      </span> 
      <span v-if="colony.name === getEuropa() || colony.name === getEuropaRebalanced()" class="colony-background-color" style="margin-left: 3px;position: relative;top: -12px;">
    ...
    <div v-if="colony.name === getIoRebalanced()" class="colony-grid-container">
    ...
    <div v-if="colony.name === getLunaRebalanced()" class="colony-grid-container2">
      <div>1</div>
      <div>2</div>
      <div>4</div>
      <div>6</div>
    ...
    <div v-if="colony.name === getCeres() || colony.name === getCeresRebalanced()" class="colony-grid-container">
    ...
    <div v-if="colony.name === getCeresRebalanced()" class="colony-grid-container2">
      <div>1</div>
      <div>2</div>
      <div>3</div>
      <div>4</div>
    ...
    <div v-if="colony.name === getGanymede() || colony.name === getGanymedeRebalanced()" class="colony-grid-container">
    ...
    <div v-if="colony.name === getGanymedeRebalanced()" class="colony-grid-container2">
      <div>1</div>
      <div>2</div>
      <div>3</div>
      <div>4</div>
  ...
  <div v-if="colony.name === getEuropa() || colony.name === getEuropaRebalanced()" class="colony-grid-container">
  ...
  <div v-if="colony.name === getEuropaRebalanced()" class="colony-grid-container2">
    <div><div class="production-box"><div class="production money">1</div></div></div>
    <div><div class="production-box"><div class="production money">1</div></div></div>
    <div><div class="production-box"><div class="production energy"></div></div></div>
    <div><div class="production-box"><div class="production energy"></div></div></div>
    <div><div class="production-box"><div class="production energy"></div></div></div>
    <div><div class="production-box"><div class="production energy triple-res white-x white-x--2"></div></div></div>
    <div><div class="production-box"><div class="production energy triple-res white-x white-x--2"></div></div></div>
  </div>
