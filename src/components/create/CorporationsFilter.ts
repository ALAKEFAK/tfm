...
const originalCorpsReplacedByRebalanced: Array<CardName> =[];
REBALANCED_CARD_MANIFEST.corporationCards.factories.forEach((cf) => {
  originalCorpsReplacedByRebalanced.push(cf.cardName.replace('(⚖)', '') as CardName);
});
...
export const CorporationsFilter = Vue.component('corporations-filter', {
  ...
  data: function() {
    ...
    return {
      ...
      selectedCorporations: [
        ...
        ...this.rebalancedExtension ? cardsByModuleMap.get(GameModule.Rebalanced)! : [],
      ].filter((corp) => !(this.rebalancedExtension && originalCorpsReplacedByRebalanced.includes(corp))) as Array<CardName> | boolean /* v-model thinks this can be boolean */,
  ...
  methods: {
    ...
    selectAll: function(group: string) {
      const items = this.getItemsByGroup(group).filter((corp) => !(this.rebalancedExtension && originalCorpsReplacedByRebalanced.includes(corp)));
    ...
    removeOriginalCorpsReplacedByRebalanced: function() {
      for (const item of originalCorpsReplacedByRebalanced) {
  ...
  watch: {
    ...
    rebalancedExtension: function(enabled) {
      if (enabled) {
        this.selectAll(GameModule.Rebalanced);
        this.removeOriginalCorpsReplacedByRebalanced();
      } else {
        this.selectNone(GameModule.Rebalanced);
      }
    },
  },
  template: `
