...
import {CeresRebalanced} from '../../colonies/CeresRebalanced';
import {EuropaRebalanced} from '../../colonies/EuropaRebalanced';
import {GanymedeRebalanced} from '../../colonies/GanymedeRebalanced';
import {IoRebalanced} from '../../colonies/IoRebalanced';
import {LunaRebalanced} from '../../colonies/LunaRebalanced';
...
const rebalancedColonies: Array<Colony> = [
  new CeresRebalanced,
  new EuropaRebalanced(),
  new GanymedeRebalanced(),
  new IoRebalanced(),
  new LunaRebalanced(),
];

const namesOfColoniesToReplaceWithRebalanced = rebalancedColonies.map((c)=>c.name.replace('(⚖)', ''));
...
export const ColoniesFilter = Vue.component('colonies-filter', {
  ...
  data: function() {
    return {
      allColonies: officialColonies.concat(communityColonies).concat(rebalancedColonies),
      officialColonies: officialColonies,
      communityColonies: communityColonies,
      rebalancedColonies: rebalancedColonies,
      selectedColonies: [
        ...officialColonies.filter((colony) => namesOfColoniesToReplaceWithRebalanced.includes(colony.name) === false ),
        ...this.communityCardsOption ? communityColonies: [],
        ...this.rebalancedExtension ? rebalancedColonies: [],
  ...
  watch: {
    ...
    rebalancedExtension: function(enabled) {
      if (enabled) {
        this.selectedColonies = officialColonies.concat(rebalancedColonies).slice();
        this.selectedColonies = this.selectedColonies.filter((c) => namesOfColoniesToReplaceWithRebalanced.includes(c.name) === false);
      } else {
        this.selectedColonies = officialColonies.slice();
      }
    },
  ...
  template: `
    <div class="colonies-filter">
        ...
        <div class="colonies-filter-list">
            <h2 v-i18n>Rebalanced</h2>
            <label class="form-checkbox" v-for="colony in rebalancedColonies">
                <input type="checkbox" v-model="selectedColonies" :value="colony"/>
                <i class="form-icon"></i><span v-i18n>{{ colony.name }} - ({{ colony.description }})</span>
            </label>
        </div>
